...
| Native evidence explorer controls | The isolated native module suite passed after adding project-scoped relationship-type filtering, projected-entity inspection, and deterministic schedule review states. An authenticated Cloud Computer request with `relationship_type=follows` and a projected entity key returned HTTP 200 and rendered the evidence explorer, entity details, and source-review state. These controls expose recorded evidence only; they do not fabricate graph facts or agent anomalies. | 2026-08-17 |
